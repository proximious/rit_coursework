...
def spiral_tail( size, depth): # wrapper makes it easier for clients to use
    return spiral_aux( size, depth, 0)

def spiral_aux( size, depth, subtotal): # add parameter for partial result
    if depth == 0:
        return subtotal  # all done
    tt.fd( size)
    tt.left( 90)
...
def main():
    ...
    """
    size = 50  # 50 is magic! no explanation why
    initialize( size)
