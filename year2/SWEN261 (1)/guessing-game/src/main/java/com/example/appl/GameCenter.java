...
public class GameCenter {
  ...
  public final static String ONE_GAME_MESSAGE = "One game has been played so far with a winrate of %d percent.";
  public final static String GAMES_PLAYED_FORMAT = "There have been %d games played with a %d percent winrate.";
   ...
   */
  public void gameFinished(GuessGame.GuessResult result) {
    // do some application-wide book-keeping
    synchronized (this) {  // protect the critical code
      if (GuessGame.GuessResult.WON.equals(result)){
        gamesWon++;
      }
      totalGames++;
    }
  ...
  public synchronized String getGameStatsMessage() {
    if (totalGames > 1) {
      winRate = (gamesWon * 100) / totalGames;
      return String.format(GAMES_PLAYED_FORMAT, totalGames, winRate);
    } else if (totalGames == 1) {
      winRate = (gamesWon * 100) / totalGames;
      return String.format(ONE_GAME_MESSAGE, winRate);
    } else {
